...
BiDAF_charCNN_BERTEnc_BERTMod | models.py | Baseline with character embedding, dropout (__0.2__) leaving BERT encoder dropout to 0.1 `--drop_prob 0.2` and BERT-encoder `--hidden_size 256` instead of RNN-encoder with 4 layers. Model layer is a BERT encoder as well (6 layers). 60 epochs (`--num_epochs 60`) | overfit  | save/train/BiDAF_charCNN_BERTEnc_BERTMod-03/best.pth.tar |00.83 | 89.94 | 83.18|94.71 | 03.57| 57.56 | 54.09 |  64.38|
BiDAF_charCNN_BERTEnc_BERTMod | models.py | Baseline with character embedding, dropout (__0.2__) leaving BERT encoder dropout to 0.1 `--drop_prob 0.2` and BERT-encoder `--hidden_size 256` instead of RNN-encoder. BERT encoder has 3 layers (instead of 6) and d_ff, the depth of feed-forward layer, is ~depth of embedding (instead of 2048). Modeling layer comes with another BERT encoder. 60 epochs (`--num_epochs 60`) |   | save/train/BiDAF_charCNN_BERTEnc_BERTMod-04/best.pth.tar | xx | xx | xx| xx| xx | xx | xx | xx|